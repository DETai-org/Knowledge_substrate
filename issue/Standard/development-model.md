...
# Universal GitHub Development Model 
(Issue → Sub-Issue → PR)
...
**Issue (Initiative) → Sub-Issue  (Work Package) → Tasks (Checklist) → Implementation Steps**.
...
**Ключевой принцип:** Initiative — это про **навигацию**, а не про реализацию. В целом Issue описывает **вектор развития репозитория**: по завершении Issue через выполнение её Sub-Issues система переходит в новое функциональное состояние —  что можно рассматривать как следующая версия проекта/репо
...
Иерархии в уровнях Issue / Sub-Issue / Task / Subtask
...
Завершение issues (Initiative) может идти неделями в него могут добавляться новые Sub-Issue

issues считается **Done** ✅, когда:
- все Sub-Issue завершены (смёрджены), и
- выполнены критерии завершения на уровне Initiative.

"При добавлении новых Sub-Issue убедись, что они соответствуют целям и остаются в рамках этой issues (Initiative). Если новые Sub-Issue выходят за рамки, необходимо создать новую issues, чтобы сохранить чёткую навигацию
