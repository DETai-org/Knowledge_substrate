...
Пример целостного шаблона с линиями‑разделителями между задачами:
```markdown
...
## 4) Document package (обязательно при закрытии)

Перед закрытием Sub-Issue необходимо подготовить связанный пакет документов:
ADR, Guide, Explanation (если существует), Policy (если существует).
Сначала создаются/обновляются все необходимые файлы, после чего в Sub-Issue
добавляется раздел `## Document package` с корректными относительными ссылками
на каждый документ и перекрёстной связностью между ними.
