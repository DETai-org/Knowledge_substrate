...
Помимо фиксации нового имени в списке, этот workflow обеспечивает актуализацию данных для сайта: обновлённый [`list_publication_blogs.json`](../../knowledge_core/source_of_truth/docs/publications/blogs/list_publication_blogs.json) попадает в репозиторий [`DETai-org/sites`](https://github.com/DETai-org/sites) (копируется в `packages/blog-index/` в рамках синхронизации блогов). Таким образом, процесс одновременно обновляет реестр публикаций и подготавливает JSON-индекс для сайта.
