...
## 🎯 Задача
Появился новый пост в хранилище блог‑постов. Найди актуальный пост - последний файл в каждой папке.
Если сомневаешься Какой файл использовать Посмотри на сайте опубликованные посты если в хранилище этот файл есть а на сайтах нет значит это он Но обычно Ориентируйся лучше на новизну по дате,  
Если НОВЫЕ посты есть в обеих папках — ДЕЛАЙ на оба сайта).

Ты работаешь в монорепозитории `sites`. Твоя задача — **инфраструктура поста** (frontmatter + конфигурация + языковые файлы), **без изменения текста поста**.
...
- Поддерживай оба сайта, если исходные посты лежат в двух папках docs.
- 
____
