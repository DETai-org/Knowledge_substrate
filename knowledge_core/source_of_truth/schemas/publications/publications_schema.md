...
`administrative.channels` может содержать **один или несколько** сайтов. Если указано несколько, файл хранится в каждой соответствующей папке `docs/publications/blogs/<TARGET_SITE>/` внутри `Knowledge_substrate` и затем синхронизируется в сайт.
