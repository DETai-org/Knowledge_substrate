...
## 4) Как хранится и используется routeSlug у постов

Данные добовляються в frontmatter в каждый файл  типа: de.md, cn.md 
Локализованный slug поста хранится в frontmatter каждого языкового файла (ru.md, en.md, de.md и т.д.). Это поле называется routeSlug (в единственном числе), потому что каждый файл соответствует одному языку и содержит одно значение slug для этого языка. 
postId при этом не меняется и остаётся стабильным идентификатором поста.

Пример для английского файла en.md:
...
Список поддерживаемых языков см. в `post-taxonomy-overview.md` (раздел 3).
...
Общие правила формата slug см. в `route-slugs-policy.md` (раздел 6).
...
### 6.4. Рекомендации по созданию `routeSlug`
Отдельные рекомендации по качеству и содержанию локализованных slug:

- **Краткость важна для SEO и удобства восприятия**, особенно при длинных заголовках.
- При этом **ключевые слова должны сохраняться** в `routeSlug`.
- **Краткие и чёткие URL** с основными ключевыми словами лучше воспринимаются
  как пользователями, так и поисковыми системами.
- **SEO‑ключевые слова должны быть включены** в каждом `routeSlug`.
