...
Изменение канонического продукта запускает проверку зависимых представлений:

| Что изменилось | Что обязательно проверить |
|---|---|
| Ценностная значимость или аудитория | Видимое обещание страницы, два GEO-вопроса, SEO title/description и structured data |
| Intended / excluded use или риск | Публичные ограничения, Trust-текст, legal-страницы и support-сценарии |
| Публичная версия или доступность | Статус на сайте, CTA, каталог, release notes и документацию начала работы |
| Основной пользовательский сценарий | Инструкции, FAQ, analytics events и продуктовые показатели |
| Ресурсный или экономический смысл | Эшелон, описание вклада, metrics source и offer/business logic |
| Закрытие или пауза продукта | CTA, status labels, индексацию, поддержку и архивный статус канонического узла |

Эта проверка является частью Documentation Architecture производственного цикла. Она выполняется при публичном `v1.0` и при каждом следующем релизе, который меняет смысл, аудиторию, доступ, ограничения или ключевой пользовательский результат. Patch-релиз без такого изменения не требует механического переписывания GEO/SEO.
