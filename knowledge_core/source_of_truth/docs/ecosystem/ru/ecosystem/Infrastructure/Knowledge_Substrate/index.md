...
links:
  external_links:
    - type: "MkDocs_ru"
      url: "https://docs.detai-x.com/ru/ecosystem/Infrastructure/Knowledge_Substrate/"
  document_links:
    - schema: ""
      link_type: ""
      linked_document_id: ""
...
**Knowledge Substrate** — инфраструктурная система, которая хранит
канонические документы об экосистеме DETai — включая страницу, которую вы
сейчас читаете, — и создаёт из них представления для людей и машин.
...
Этот сайт базы знаний, собранный на MkDocs, — одно из таких представлений. Он не
исчерпывает систему: тот же источник преобразуется для поиска, retrieval,
индексации и работы через `detai_core`.

## Как устроен репозиторий

- `source_of_truth/` хранит первичные документы, metadata, словари и связи;
- `ingest_pipeline/` разбирает и проверяет канон, подготавливая
  структурированные записи и индексы;
- `operational_store/` описывает производные хранилища и модели доступа;
- MkDocs собирает из исходных Markdown-файлов публичную библиотеку знаний.

Производные представления можно пересобрать из source of truth. Поэтому сайт,
поисковый индекс или база запросов не становятся отдельными владельцами знания.
