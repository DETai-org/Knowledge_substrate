...
- удержать юридическую и SEO-рамку публичного присутствия;
- связать публикационный слой, платформенные проекты и аккаунтные сценарии в одном web-пространстве.
...
`sites` отвечает за frontend, публичные маршруты, визуальную систему, локализацию, SEO, legal pages, analytics/consent runtime и web-интеграции.

Он не является источником истины для identity, intake storage, Telegram-ботов, runtime API или серверной инфраструктуры. Эти слои живут в соседних runtime/ops-репозиториях и подключаются через API-контракты.

## Документационная граница

В репозитории `sites` должны оставаться только документы, которые помогают работать с кодом: README, локальные guide, технические contracts, policies и machine-readable schemas.

Смысловые explanation, standards и ecosystem-level policies должны жить в Knowledge Substrate. Операционные параметры и исторические логи должны фиксироваться в ClickUp/reference-слое, а не накапливаться в проектном репозитории.
