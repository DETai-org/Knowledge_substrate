...
links:
  external_links:
    - type: "MkDocs_ru"
      url: "https://detai-org.github.io/Knowledge_substrate/ru/ecosystem/Management_layer/Docs-Ecosystem/documentation-architecture/"
  document_links:
    - schema: ""
      link_type: ""
      linked_document_id: ""
...
Onboarding присутствует в ClickUp в двух разных ролях:

| Контейнер | Для кого и для чего | Что попадает в `logs` |
|---|---|---|
| Space `Onboarding` | среда, в которой новый участник проходит tutorial-задачи | прогресс и результаты прохождения onboarding участником |
| Folder `Management Layer / Onboarding` | управляющий контур владельцев и сопровождающих onboarding | проектирование, обновление и координация onboarding |

Эти Lists `logs` не взаимозаменяемы. Участническое исполнение не записывается в management-журнал, а работа владельцев над системой onboarding не подменяется записью в пространстве прохождения. Если management-изменение материально связано с конкретным прохождением, обе записи создаются в своих контейнерах и взаимно ссылаются.

Конкретные ClickUp ID принадлежат operational navigation map и project routes, а не концептуальной документации. Contract классификации и связей для дневных записей определён в [реестре metadata schemas](metadata_schema_registry.md#contract-log-summary).
