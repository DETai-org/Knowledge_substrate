...
links:
  external_links:
    - type: MkDocs_ru
      url: https://docs.detai-x.com/ru/ecosystem/Management_layer/Docs-Ecosystem/functions_of_documents/
  document_links:
    - schema: ecosystem
      link_type: defines
...
`Tutorial` определяет **функцию документа, а не обязательное место хранения**. Экосистемный onboarding tutorial обычно живёт в репозитории Onboarding и ведёт человека через общий маршрут участия. Project-specific tutorial может храниться рядом с кодом проекта, если обучает именно этому инструменту или процессу. Если текст одновременно показывается внутри интерфейса продукта, проектный tutorial может быть source of truth для этой runtime-проекции, чтобы Git-документ и встроенная помощь не расходились.
