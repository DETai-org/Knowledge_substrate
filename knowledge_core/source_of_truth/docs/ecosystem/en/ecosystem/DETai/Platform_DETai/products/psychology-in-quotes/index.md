---
title: Psychology in Quotes
description: Translation placeholder preview for the Psychology in Quotes documentation page.
robots: noindex, follow
translation_placeholder: true
canonical_original: /ru/ecosystem/DETai/Platform_DETai/products/psychology-in-quotes/
descriptive:
  status: draft
hide:
  - navigation
  - toc
---

<section class="translation-placeholder-page" markdown>

<p class="translation-placeholder-page__eyebrow">Translation preview · English</p>

# Psychology in Quotes

<p class="translation-placeholder-page__lead">
This route is ready, while the full English edition of the document is still being prepared.
</p>

!!! warning "This page is currently available in Russian"

    We’re preparing the English version. You can open the Russian original of this same document now.

    [Open the Russian original](/ru/ecosystem/DETai/Platform_DETai/products/psychology-in-quotes/){ .md-button .md-button--primary }

!!! info "Get the English release update"

    Be the first to know when DETai documentation is available in English. Follow our Telegram channel for release updates.

    <a class="md-button translation-placeholder-page__telegram" href="https://t.me/detai_en"><img class="translation-placeholder-page__telegram-flag" src="/assets/images/flags/gb.svg" alt="" aria-hidden="true"><span>Subscribe on Telegram</span><img class="translation-placeholder-page__telegram-mark" src="/assets/images/telegram-mark.svg" alt="" aria-hidden="true"></a>

</section>
