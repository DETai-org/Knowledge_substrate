...
В рамках [[work-model|Work Model]] сложная работа организуется как:
...
Epic Issue ведётся как рабочий процесс в GitHub и оформляется по стандарту [[issue-contract]].  
Epic дробится на Sub-Issue (Work Packages) по стандарту [[sub-issue-contract]].
...
```
🫱 *Нужно следить, чтобы этот шаблон соответствовал той структуре, которая указана в стандарте* [[issue-contract]]
...
Для ускорения создания `Epic_issue` файла используется промпт: [[create-epic-issue-release|Create Epic Issue Release]]
