...
- Цикл указывается **только если пользователь явно задал `cycle_ids`**.
  Если поле отсутствует или пустое, значит пост **не входит в цикл**.
- При задании `cycle_ids` цикл выбирается из словаря `controlled_vocabularies/cycle.json`
  и должен соответствовать выбранному сайту поста.
  Если цикл пока без привязки к сайту, его нельзя использовать до назначения.
