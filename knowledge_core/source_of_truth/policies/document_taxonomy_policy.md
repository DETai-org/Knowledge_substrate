...
### 4.1 Привязка rubrics к сайту

Для документов, где используются rubrics, набор rubrics определяется выбранным сайтом:

* сайт **обязательно** передаётся в исходных данных от пользователя (`administrative.channels`);
* rubrics выбираются **только** из списка, соответствующего этому сайту;
* смешивать rubrics из разных сайтов **нельзя**.

---

### 4.2 Исключение для Research Publication
...
Цикл выбирается только из словаря `controlled_vocabularies/cycle.json` и должен соответствовать сайту поста.
