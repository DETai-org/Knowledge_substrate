...
`routeSlugs` для **всех поддерживаемых языков**. Черновиков в репозитории не храним.
...
Эти правила являются **едиными** и применяются также к slug постов.
См. `post-slugs-policy.md` для правил, специфичных для постов.
...
Локализация slug постов — отдельный шаг и отдельная политика, потому что требует миграции структуры хранения и маршрутизации. См. `post-slugs-policy.md`. ❗❗
